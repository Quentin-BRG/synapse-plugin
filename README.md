...
This repository distributes version `0.2.0` of the `synapse`
...
The plugin requests only the scopes used by its current text-based workflows:
...
- `synapse:content:write` creates or updates Collections and KnowledgeUnits
  after your approval;
- `synapse:learning:write` sets your private targets and records
  evidence-backed learning events and state.

The package does not request file, social, or sharing write scopes. Scopes do
not replace server-side entity authorization: the Synapse service enforces
access and privacy for every request.
...
The current workflows are text-only. Document upload is not available in this
release.
